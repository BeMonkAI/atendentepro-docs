...
HistoryWindow(
    ...
    on_agent_switch=lambda prev, nxt: None,
)
```
...
| `on_agent_reset` | `Optional[Callable[[str, List[Dict]], None]]` | `None` | ✅ **ativo (Phase 4.3, v0.21.0)** | Callback **cross-turn** disparado no início de cada turno quando o último agente que produziu output **NÃO** foi o Triage. Recebe `(agent_name, messages)` mutável — caller pode editar a lista in-place antes da sumarização/truncamento. Exceções são logadas (WARNING) e ignoradas. |
| `on_agent_switch` | `Optional[Callable[[str, str], Awaitable[None] \| None]]` | `None` | ✅ **ativo (issue #121)** | Callback **intra-run** disparado a cada handoff entre agentes dentro de `Runner.run` (ex.: `Triage Agent` → `Knowledge Agent`, `Knowledge Agent` → `Answer Agent`). Recebe `(prev_agent_name, next_agent_name)`. Pode ser síncrono ou async (awaited automaticamente). Wire via SDK `RunHooks.on_handoff`. Útil para telemetria de routing. Exceções são logadas (WARNING) e ignoradas. |
...
## Exemplo 4 — Telemetria de routing intra-run com `on_agent_switch` (issue #121)

`on_agent_switch` complementa `on_agent_reset`:

| | `on_agent_reset` | `on_agent_switch` |
|---|---|---|
| **Escopo** | Cross-turn (entre `Runner.run` calls) | Intra-run (dentro de um `Runner.run`) |
| **Quando dispara** | Início do turno, se último agente ≠ Triage | A cada handoff entre agentes durante o run |
| **Assinatura** | `(agent_name, messages)` | `(prev_agent_name, next_agent_name)` |
| **Mutação** | Pode editar `messages` in-place | Apenas observação |
| **Wire** | `apply_history_window` lê `last_agent_name` | SDK `RunHooks.on_handoff` |
| **Sync/Async** | Sync | Sync ou async (await automático) |

```python
import logging

logger = logging.getLogger("atendentepro.routing")

async def _audit_handoff(prev: str, nxt: str) -> None:
    # Async porque queremos publicar em queue/Redis sem segurar o run.
    await audit_queue.publish({"prev": prev, "next": nxt})
    logger.info("handoff %s -> %s", prev, nxt)

network.history_window = HistoryWindow(
    on_agent_switch=_audit_handoff,
)
```

Em uma sessão Reports Picker (Knowledge slot), uma execução típica produziria:

```
handoff Triage Agent -> Knowledge Agent     # T1: Triage roteou para Knowledge
handoff Knowledge Agent -> Answer Agent     # Knowledge terminou a busca, passou para Answer
```

Útil para:
- **Observabilidade de routing** — quanto tempo cada agente segura o run.
- **Detectar loops de handoff** (`Triage` → `Flow` → `Triage` → `Flow` ...) para tunar prompts.
- **Telemetria** para dashboards (Grafana / App Insights).

**Não suporta** (limitação atual):
- Injeção de mensagens no input do próximo agente (a issue original sugeria, mas o SDK não expõe isso de forma estável). Para state recovery entre agentes, ver [`run_with_history`](09_opcionais_run_with_history.md) que preserva tool outputs replayable.
