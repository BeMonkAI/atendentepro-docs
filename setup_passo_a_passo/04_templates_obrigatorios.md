...
| `scheduling_config.yaml` | Agendamento de consultas (v0.25.0+, opt-in via `include_scheduling=True`). Suporta per-action toggles (`allow_create`/`allow_reschedule`/`allow_cancel`, v0.31.0+) e multi-location (v0.37.0+). Ver Passo 17 e 18. |
| `farewell_config.yaml` | Encerramento gracioso da conversa (v0.32.0+). Reconhece despedidas e fecha o turno sem voltar ao Triage. Ativo por padrão em `create_standard_network()`; desabilitar com `include_farewell=False`. |
| `mcp_config.yaml` | Servidores MCP como fonte de tools (opt-in, #491). So `sse` / `streamable_http`; URL e headers via env; `stdio` e proibido. Ver Passo 25. |
