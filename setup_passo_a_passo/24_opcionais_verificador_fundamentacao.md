...
self-evaluation, não verificação externa. Timeout vem de `tier2_timeout_s`
(default 2.5s), que desde a #503 limita **apenas o probe**. O juiz Tier 2 tem
o seu próprio, `tier2_judge_timeout_s` (default 8.0s), porque ele é de
propósito o modelo mais lento e mais capaz: um orçamento só para os dois
cortaria chamadas do juiz que hoje completam sem problema. Falha do probe
...
## Claim qualitativo closed-book (#478, opt-in)

Com `closed_book_qualitative: true`, uma asserção factual QUALITATIVA sem
dígito — liderança/superlativo/comparação ("a marca Acme é a líder da
categoria") — dita SEM contexto recuperado passa a gerar `warn` terminal no
Tier 0 (categoria `closed_book_qualitative_claim`, `error_mode=c`), espelhando
o caminho numérico `closed_book_claim`. Fecha o último gap documentado do
Tier 0 (#422): antes, esses drafts saíam `allow`/`not_applicable_no_rag` sem
verificação, porque o Tier 0 ancorava só em token numérico.

O gatilho é zero-LLM e exige DOIS âncoras na MESMA frase não-interrogativa:
uma construção assertiva (cópula + artigo + superlativo/líder, ou verbo de
liderança — lidera/domina/supera/à frente de; equivalentes em inglês) E um
substantivo de domínio de mercado (mercado, categoria, setor, concorrência,
ranking...). A conjunção mantém fora recomendações ("essa é a melhor opção
para o seu caso"), cortesia e perguntas. Complementar ao probe F-LP2: o probe
pega a subfamília "untrue" (p_true baixo); este gate pega asserções sem fonte
mesmo quando o modelo as considera verdadeiras (true-but-ungrounded).

Default OFF (byte-for-byte pré-#478) — ao ligar, drafts que antes passavam
`allow` viram `warn`, então valide a precisão no seu tráfego em shadow antes
de qualquer uso enforce.

```yaml
groundedness_verifier:
  closed_book_qualitative: true
```
