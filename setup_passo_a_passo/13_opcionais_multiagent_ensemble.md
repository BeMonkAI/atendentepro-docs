...
### Injetando agentes pre-construidos (`custom_agents`)

A fabrica monta agentes "bare" (`Agent(name=..., instructions="")`) por
padrao — suficiente para testes que monkey-patcham o runner, mas em
producao voce quase sempre quer instructions, tools e providers
proprios. Use `custom_agents` para entregar suas instancias prontas:

```python
from agents import Agent
from atendentepro import create_parallel_network

persona_a = Agent(
    name="persona_a",
    instructions="Voce e um analista value-investing focado em margin of safety.",
    tools=[my_dcf_tool],
    model="gpt-4o",
)
coordinator = Agent(
    name="portfolio_manager",
    instructions="Voce sintetiza signals de N personas em uma unica recomendacao.",
    model="gpt-4o",
)
...
- Cada chave em `custom_agents` precisa estar declarada no bloco
  `agents:` do YAML — typos viram `ValueError` no build.
- Nomes do YAML que voce nao injetar caem no fallback bare; util pra
  injecao parcial (ex: passar so as personas e deixar o coordinator
  bare em smoke tests).
- A factory **nao** muta os agentes recebidos. Se quiser que o agente
  retorne um `output_schema` automaticamente, use `agent.clone(output_type=...)`
  antes de passar — `invoke_agent` ja faz esse clone interno por
  invocacao quando `output_schema` esta declarado no YAML.
