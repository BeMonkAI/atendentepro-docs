...
## Storage backend (file vs postgres) — opcional

A partir de `grkmemory>=1.9.0` o armazenamento da memória é plugável e selecionado por env (sem mudar código). O default é `file` (flat-file, single-process) — deploys existentes ficam inalterados.

Para externalizar a memória num Postgres + pgvector (cross-process, remove o pin de instância única + volume persistente):

```env
GRKMEMORY_STORAGE_BACKEND=postgres
GRKMEMORY_POSTGRES_DSN=postgresql://user:pass@host:5432/db
# GRKMEMORY_EMBEDDING_DIM=1536   # opcional (default 1536)
```

E instalar o extra: `pip install atendentepro[memory-postgres]`. O `create_grk_backend` repassa a config para o `GRKMemory`, que seleciona o backend automaticamente — `runner.py` (search-before / save-after) não muda.
