...
2. **Prompt**: o agente recebe um bloco `[RESTRIÇÕES]` listando o que NÃO está disponível e orientando o usuário a procurar contato direto com o estabelecimento.
...
Quando o consumer passa `create_scheduling_agent(custom_instructions="...")`, o bloco `[RESTRIÇÕES]`/`[INDISPONÍVEL]` **não** é injetado — o consumer assume responsabilidade pelo prompt. A filtragem de tools continua aplicada, então o LLM ainda não enxerga ferramentas desabilitadas.
