...
- **Mitigação recomendada para heurísticos customizados**: detectar se a última mensagem do assistant terminou com `"?"` e, em caso afirmativo, aceitar respostas curtas ou itens de uma allowlist configurável (`sim/não/ok/1/2/esse/abre/cancela/...`). Acompanhar a issue [#122](https://github.com/BeMonkAI/atendentepro/issues/122) para a expansão futura da API (`run_input_guardrails(..., history=...)`).
- **Alternativa moderna (v0.43.0+):** o [`SpecialistGuardrail`](22_opcionais_specialist_guardrail.md) (PR #285) trata short follow-ups em uma rule dedicada (regra 8) sem precisar de heurístico custom — basta passar `history` no `run_specialist_guardrail`. Recomendado para tenants regulados (saúde / financeiro / jurídico) onde o veredito `warn`/`escalate` agrega valor além de `is_in_scope: bool`.
