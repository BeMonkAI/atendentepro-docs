# AtendentePro — Public Documentation & Examples

> Public mirror of documentation and examples for [AtendentePro](https://pypi.org/project/atendentepro/) — v0.60.0.
> Source library lives at [BeMonkAI/atendentepro](https://github.com/BeMonkAI/atendentepro) (private).

## For humans

- [Setup guide](setup_passo_a_passo/) — step-by-step install & first network
- [Feature examples](examples/) — concept-driven examples by feature
- [Runner scripts](run/) — provider-neutral end-to-end scripts

## For AI assistants

- **`llms.txt`** — short index ([llmstxt.org](https://llmstxt.org) convention)
- **`llms-full.txt`** — entire bundle concatenated; ideal for LLM context
...
(e.g. PyPI `0.21.0` ↔ git tag `v0.21.0`). The `main` branch tracks
...
| **Agendamento** (v0.25.0+, opt-in) | Scheduling Agent canônico: marcação, cancelamento e reagendamento de consultas; per-action toggles (`allow_create`/`allow_reschedule`/`allow_cancel`) e multi-location |
| **Encerramento Gracioso** (v0.32.0+) | Farewell Agent reconhece despedidas (`"tchau"`, `"valeu"`, `"encerrar"`) e fecha a conversa com cordialidade sem retornar ao Triage |
...
| **SpecialistGuardrail** (v0.43.0+, opt-in) | Veredito multi-dimensional substituindo `is_in_scope: bool` por `decision` ∈ {allow, warn, escalate, block} + `risk_level` + `category` + `confidence`. Function tools `detect_pii` + `classify_intent`. Validado em paper JAES (90% acuracia em D1 v0.2 vs. 74% do classificador binário). Detalhes: [Passo 22](docs/setup_passo_a_passo/22_opcionais_specialist_guardrail.md) + [fluxograma](docs/fluxogramas/04_specialist_guardrail.md). |
| **Verificador de Fundamentação** (v0.52.0+, opt-in, shadow) | Camada OUTPUT-side: depois que a resposta é gerada e antes de enviar, verifica se ela está **fundamentada** no contexto recuperado (RAG) via cascata Tier 0 (heurística zero-LLM) + Tier 1 (entailment). Emite GroundednessVerdict (decision allow/warn/escalate/block + faithfulness). NÃO é um guardrail (guardrails são input-side). groundedness ≠ verdade. Detalhes: [Passo 24](docs/setup_passo_a_passo/24_opcionais_verificador_fundamentacao.md) + [fluxograma](docs/fluxogramas/06_verificador_fundamentacao.md). |
...
**👉 [github.com/BeMonkAI/atendentepro-docs](https://github.com/BeMonkAI/atendentepro-docs)**
...
O arquivo `llms-full.txt` segue a convenção [llmstxt.org](https://llmstxt.org) — todas as docs concatenadas em um único markdown otimizado para contexto de LLM. Cada release no PyPI tem uma tag matching no repo de docs (ex: `atendentepro==0.21.0` ↔ `git tag v0.21.0`).

---

## 📋 Índice

- [Documentação e Exemplos](#-documentação-e-exemplos)
...
- [CLI (Agent Builder)](#️-cli-agent-builder)
...
🔧 **Padrão para campos custom do cliente** (`empresa`, `aad_object_id`, etc.): [docs/client_context_patterns.md](docs/client_context_patterns.md) — quando criar uma subclasse de `UserContext` para tipar campos específicos do diretório corporativo.
